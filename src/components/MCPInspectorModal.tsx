  ...
  useEffect(() => {
    ...
    if (selectedTool === 'ga4_run_report') {
      ...
      setToolParamsJson(JSON.stringify({
        ...
        query: 'SELECT * FROM `bigquery-public-data.usa_names.usa_1910_current` LIMIT 10;',
        maxResults: 20
      }, null, 2));
    } else if (selectedTool === 'bigquery_list_projects') {
      setToolParamsJson(JSON.stringify({}, null, 2));
    } else if (selectedTool === 'bigquery_list_datasets') {
      setToolParamsJson(JSON.stringify({
        projectId: 'bigquery-public-data'
      }, null, 2));
    } else if (selectedTool === 'bigquery_list_tables') {
      setToolParamsJson(JSON.stringify({
        projectId: 'bigquery-public-data',
        datasetId: 'usa_names'
      }, null, 2));
    } else if (selectedTool === 'bigquery_get_table_schema') {
      setToolParamsJson(JSON.stringify({
        projectId: 'bigquery-public-data',
        datasetId: 'usa_names',
